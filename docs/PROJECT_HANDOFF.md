...
<!-- START 2026-04-27 公众号回复前价格刷新一致性修正 -->
## 2026-04-27 公众号回复前价格刷新一致性修正

本次修正范围只在推荐运行时出口层，不改 JD 通信边界、不改数据库结构、不重写价格解析主逻辑。

已确认价格解析链路和 H5 canonical renderer 可以正确展示 `到手约 / 京东官网价 / 可省`。问题出在公众号被动回复文本和今日推荐消息生成早于 H5 详情页价格刷新，导致服务号文字仍使用旧目录价，而 H5 页面使用刷新后的京东价格。

规则固化：
1. `find_product_entry` 与 `today_recommend` 在生成用户可见文本、news、H5 batch 链接前，必须先对候选商品做小批量价格刷新。
2. 价格刷新后再调用统一 `_format_price_line()` / `_news_value_line()` / `_price_advantage_main()`，禁止另起一套文本价格格式。
3. `price_verified_at` 只能表示“查过价格”，不能作为推荐价值排序信号。
4. 有可省金额的商品优先展示；可省为 0 的商品只能在缺少更优候选时作为兜底。
5. 旧微信消息不会自动更新，测试必须重新点击菜单生成新消息。
<!-- END 2026-04-27 公众号回复前价格刷新一致性修正 -->
<!-- START 2026-04-27 H5分支收口与落地页安全价 -->
## 2026-04-27 H5分支收口与落地页安全价

本次修正只处理用户出口层：公众号文本、H5展示、更多类似商品路由、推荐排序展示口径。不改后台定时任务主逻辑，不改 JD 通信基础边界。

问题确认：
1. 京东联盟 `purchasePriceInfo.purchasePrice` / `priceInfo.lowestCouponPrice` 可能是领券后价，但当前跳转短链不能保证用户落地页已经领券，因此不能直接作为大字“到手价”承诺。
2. `更多类似商品` 曾进入旧 H5 测试页，出现“智省优选（测试版）/ user_id=undefined”。
3. 排序不能被 coupon-derived 价差拉偏；用户可见价格必须优先与京东落地页大价格一致。

固化规则：
1. 展示层默认使用 landing-page-safe 价格：京东页面可见价优先。
2. 未确认能带券跳转前，coupon-derived 差价不得作为大字到手价和排序高权重。
3. 价格优势展示顺序为：可省金额 -> 到手约 -> 京东官网价 -> 热销。
4. `more-like-this` 必须走 canonical batch H5 renderer，禁止回到旧测试页。
<!-- END 2026-04-27 H5分支收口与落地页安全价 -->
