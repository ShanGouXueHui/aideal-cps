...
## 17. 点击归因跳转网关（进行中）
当前开始把 /promotion/redirect 升级为真正的点击归因网关：
- 先记录 click_logs
...
### 已处理
- 接入 `app.api.wechat_recommend_h5` 路由，修复 `/h5/recommend/{product_id}` 未挂载导致的 404。
- 推荐文案恢复为商用模板：
  - 标题保留 `🔥 今日推荐 3 个，可直接购买：`
  - 商品编号与第一行同一行：`【1】商品标题`
  - 保留 `图文详情`、`下单链接`
...
- 为 `/promotion/redirect` 增加异常日志，方便定位 500。
...
### 下一步建议
1. 单独修复 `/promotion/redirect` 500 的根因，重点看：
